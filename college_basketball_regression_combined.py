...
def prepare_bracket_with_factors(bracket_df: pd.DataFrame, factor_df: pd.DataFrame) -> pd.DataFrame:
    ...
    df['Off_vs_opp_def']  = df['Team_Offensive']  - df['Opp_Defensive']
    df['Def_vs_opp_off']  = df['Team_Defensive']  - df['Opp_Offensive']
    ...
    factor_cols = ["Team_Pace","Opp_Pace","Team_NET","Opp_NET", "Team_Rebounds","Opp_Rebounds"]
...
def gather_training_data(past_years):
    frames = []
    for y in past_years:
        # 1. Load bracket + factor files (Pace, NET, OReb, etc.)
        bracket_df = load_bracket_results(y)
        factor_df  = load_factors_for_year(y, ["pace","net_rating","rebounds","offensive","defensive"])

        # 2. Merge to get per-matchup data
        df_year    = prepare_bracket_with_factors(bracket_df, factor_df)

        # 3. Add a binary outcome: 'Team_Won'
        #    (1 if TeamName side was the actual winner)
        df_year["Team_Won"] = (df_year["WinnerName"] == df_year["TeamName"]).astype(int)
        
        df_year = exclude_top_seeds(df_year, top_n=3)

        frames.append(df_year)

    # 4. Concatenate all years into one DataFrame
    train_df = pd.concat(frames, ignore_index=True)
    return train_df

def exclude_top_seeds(df: pd.DataFrame, top_n: int = 5) -> pd.DataFrame:
    """
    Returns a filtered DataFrame that excludes any matchup
    where Team_Seed <= top_n OR Opp_Seed <= top_n.
...
if __name__ == "__main__":
    ...
    train_df = gather_training_data(past_years)

    #train_df = exclude_top_seeds(train_df)

    # Some columns you might have:
    # "Pace_Diff", "NET_Diff", "OReb_Diff", ...
    features = ["Team_Seed","Opp_Seed","Team_Pace","Pace_Diff", "Team_NET","Team_Rebounds","Team_Offensive","Team_Defensive","Off_vs_opp_def","Def_vs_opp_off"]
    X_train = train_df[features]
    y_train = train_df["Team_Won"]
    ...
    # Evaluate on the same training set (or do cross-validation)
